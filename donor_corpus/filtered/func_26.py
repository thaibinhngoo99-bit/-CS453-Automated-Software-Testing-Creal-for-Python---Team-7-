def plot_autocorr(data, var_names=None, filter_vars=None, max_lag=None, combined=False, grid=None, figsize=None, textsize=None, labeller=None, ax=None, backend=None, backend_config=None, backend_kwargs=None, show=None):
    ...
    plotters = filter_plotters_list(list(xarray_var_iter(data, var_names, combined)), 'plot_autocorr')
    rows, cols = default_grid(len(plotters), grid=grid)
    autocorr_plot_args = dict(axes=ax, plotters=plotters, max_lag=max_lag, figsize=figsize, rows=rows, cols=cols, combined=combined, textsize=textsize, labeller=labeller, backend_kwargs=backend_kwargs, show=show)
    if backend is None:
