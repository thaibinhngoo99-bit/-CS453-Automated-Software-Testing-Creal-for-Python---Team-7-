def proc_dwi(config, args):
    ...
    for scan in iterconfig(config, 'dwi'):
        ...
        fbase = templ.safe_substitute(sub=legal.sub('', args.subject), ses=legal.sub('', args.session), acquisition=scan.get('acquisition', None), direction=scan.get('direction', None), run=scan.get('run', None), modality=scan.get('modality', None))
