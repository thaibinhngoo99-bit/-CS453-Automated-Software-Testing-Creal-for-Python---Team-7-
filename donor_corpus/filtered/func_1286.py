...
def test_segment_user_data_view(site, client, mocker, django_user_model):
    ...
    rule2 = VisitCountRule(counted_page=site.root_page.get_last_child(), segment=segment)
    rule1.save()
    rule2.save()
    mocker.patch('wagtail_personalisation.rules.VisitCountRule.get_user_info_string', side_effect=[3, 9, 0, 1])
