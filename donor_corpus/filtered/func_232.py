def py_wrap_yggdrasil_learners(name=None, learner_deps=[]):
    ...
    native.genrule(name=name + '_copy_cc_main', outs=[local_cc_main], srcs=[wrapper_package + ':wrapper_main.cc'], cmd='cp $< $@')
    tf_cc_binary(name=wrapper_name, copts=tf_copts(), linkopts=if_not_windows(['-lm', '-Wl,-ldl']) + rpath_linkopts_to_tensorflow(wrapper_name), srcs=[':' + local_cc_main], deps=[wrapper_package + ':wrapper'] + learner_deps, linkstatic=1)
    native.genrule(name=run_wrapper_name, srcs=[], outs=[name + '.py'], cmd='$(location ' + wrapper_name + ') > "$@"', tools=[':' + wrapper_name] + tf_binary_additional_srcs())
    native.py_library(name=name, srcs=[name + '.py'], srcs_version='PY3', deps=['//tensorflow_decision_forests/keras:core', '@org_tensorflow//tensorflow/python', '@ydf//yggdrasil_decision_forests/model:abstract_model_py_proto', '@ydf//yggdrasil_decision_forests/learner:abstract_learner_py_proto'], data=[':' + run_wrapper_name, ':' + wrapper_name])