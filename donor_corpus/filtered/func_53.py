def plot_confusion_matrix(confusion_matrix, classes_list, normalize=True, figsize=(10, 7), fontsize=14, cmap='Blues'):
    ...
    >>> array = [[ 8458,   227,  1730],              [ 1073, 37590,  1613],              [ 2390,  1159, 17540]]
    >>> classes_list = ["A", "B", "C"]
   ...
   """
    ...
    if normalize:
        normalized_cm = np.array(confusion_matrix).astype('float') / np.array(confusion_matrix).sum(axis=1)[:, np.newaxis]
        df_cm = pd.DataFrame(normalized_cm, index=classes_list, columns=classes_list)
        plt.matshow(df_cm, fignum=0, cmap=cmap)
    else:
        df_cm = pd.DataFrame(confusion_matrix, index=classes_list, columns=classes_list)
        plt.matshow(df_cm, fignum=0, cmap=cmap)
