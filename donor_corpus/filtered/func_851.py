def test_acl_config_update(setup_scopez_server_action):
    ...
    """
    l_uri = G_TEST_HOST
    l_headers = {'host': 'monkeez.com', 'user-agent': 'gizoogle', 'waf-scopes-id': '0050'}
    ...
    try:
        ...
        print('error opening config file: %s.  Reason: %s error: %s, doc: %s' % (l_acl_conf_path, type(l_e), l_e, l_e.__doc__))
        assert False
