def _vim_vital_web_api_github_main():
    ...
    DEFAULT_INDICATOR = 'Requesting entries and converting into JSON %%(page)d/%(page_count)d ...'
    ...
    def format_exception():
        ...
        return '%s: %s at %s:%d' % (exc_obj.__class__.__name__, exc_obj, filename, lineno)
    ...
    def build_url(url, **kwargs):
        ...
        p = parse_qs(query)
        p.update(kwargs)
        return urlunparse([scheme, netloc, path, params, urlencode(p, doseq=True), fragment])
    ...
    def request_entries(url, token, indicator=DEFAULT_INDICATOR, page_start=1, page_end=0, nprocess=20, callback=None, **kwargs):
        ...
        for page in range(page_start, page_end + 1):
            queue.append([page, indicator % {'url': url, 'page_count': page_end - page_start + 1}])
        kwargs = dict(target=_request_entries, args=(lock, queue, entries_per_pages, url, headers, callback))
        workers = [Thread(**kwargs) for n in range(nprocess)]
        for worker in workers:
            worker.start()
        for worker in workers:
            worker.join()
        exceptions = list(filter(lambda x: not isinstance(x, list), entries_per_pages))
        if len(exceptions):
            raise exceptions[0]
        return list(chain.from_iterable(map(lambda x: x[1], sorted(entries_per_pages, key=lambda x: x[0]))))
    ...
    try:
        ...
        kwargs = {ensure_unicode(k, encoding): ensure_unicode(v, encoding) for k, v in kwargs.items()}
